...
в файле `~/.zshrc` нужно создать переменную по имени `AUTORUN_BASHLER='~/.autorun_bashler.sh'`, в этот файл мы запишем скрипт который должен выполниться при первом запуске оболочки.
