...
from bashler_py import (
    color,
    print_doc,
    read_sh_file,
    search_from_name_alias,
    search_from_value_alias,
    search_full_doc,
    search_func,
    search_shot_doc,
    self_bashler_path,
...
def manger_search_func():
    ...
    if len(similar_commands) == 1:
        ...
        print(
            "{read}команда не найдена:{reset} {0}".format(
                name_regex,
...
def search_alias():
    ...
    if type_search == "-v":
        ...
        if not is_text:
            print(
                "{read}Нет похожих значений алиаса:{reset} {0}".format(
                    name_regex,
                    read=color.read.value,
                    reset=color.reset.value,
                )
